...
const writeShim_ = (from, to, prog, args, variables) => {
  ...
  let pwsh = '#!/usr/bin/env pwsh\n'
           ...
           + '\n'
  if (shLongProg) {
    const hasProgramExeArgs = args.trim() !== ''
    pwsh = pwsh
         ...
         + '\n'
         + 'if ($MyInvocation.ExpectingInput) { # takes pipeline input\n'
         + `  $input | & "$PROGRAM_EXE" ${args} "$PROGRAM_FILE" $args\n`
         + `} elseif ($${hasProgramExeArgs} -or -not $MyInvocation.Line) { # used "-File" argument\n`
         + `  & "$PROGRAM_EXE" ${args} "$PROGRAM_FILE" $args\n`
         + '} else { # used "-Command" argument\n'
         + "  if (($MyInvocation | Get-Member -Name 'Statement') -and $MyInvocation.Statement) {\n"
         + '    $ORIGINAL_COMMAND = $MyInvocation.Statement\n'
         + '  } else {\n'
